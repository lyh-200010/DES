IP_table = [58, 50, 42, 34, 26, 18, 10, 2,  # 初始置换IP表
...
_IP_table = [40, 8, 48, 16, 56, 24, 64, 32,  # 逆初始置换IP-1表
...
yasuo1_table = [57, 49, 41, 33, 25, 17, 9,  # 选择置换1表
         1, 58, 50, 42, 34, 26, 18,
         10, 2, 59, 51, 43, 35, 27,
         19, 11, 3, 60, 52, 44, 36,
         ...
         14, 6, 61, 53, 45, 37, 29,
         21, 13, 5, 28, 20, 12, 4]
yasuo2_table = [14, 17, 11, 24, 1, 5,  # 选择置换2表
         3, 28, 15, 6, 21, 10,
         23, 19, 12, 4, 26, 8,
         16, 7, 27, 20, 13, 2,
         41, 52, 31, 37, 47, 55,
         ...
         44, 49, 39, 56, 34, 53,
         46, 42, 50, 36, 29, 32]
extend_table = [32, 1, 2, 3, 4, 5,  # 表拓展置换
   4, 5, 6, 7, 8, 9,
   8, 9, 10, 11, 12, 13,
   12, 13, 14, 15, 16, 17,
   16, 17, 18, 19, 20, 21,
...
S = [S1, S2, S3, S4, S5, S6, S7, S8]  # des算法s盒
...
def des(text,keyResult,finalTextOfBit,finalTextOfUnicode):
...
def DES(text, key, optionType):
    ...
    if optionType == 0:  # 选择的操作类型为加密
        finalTextOfChar=des(text,keyResult,finalTextOfBit,finalTextOfUnicode)
        return finalTextOfChar
    else:  # 选择的操作类型为解密
        finalTextOfChar=des(text,keyResult,finalTextOfBit,finalTextOfUnicode)
        return finalTextOfChar
